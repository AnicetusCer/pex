                    ...
                    ui.allocate_ui_with_layout(
                        ...
                        |ui| {
                            ...
                            if let Some(row) = self.rows.get(idx) {
                                ...
                                let tags_joined =
                                    if !row.genres.is_empty() { Some(row.genres.join("|")) } else { None };
                                ...
                                let better_hd_available = row.owned && !owned_is_hd && broadcast_hd;
                                ...
                                let should_dim = row.owned && self.dim_owned && !better_hd_available;
                                if should_dim {
                                ...
                                let ch = row.channel.as_deref()
                                    ...
                                    .unwrap_or_else(|| "—".into());
                                let line2 = if broadcast_hd { format!("{ch} • HD") } else { ch };
                                let tm = row.airing
                                    .map(crate::app::utils::hhmm_utc)
                                    .unwrap_or_else(|| "—".into());
                                let line3 = tm + " UTC";

                                let label_text = format!("{title}\n{line2}\n{line3}", title = title_line);

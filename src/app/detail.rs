    ...
    let panel = eg::SidePanel::right("detail_panel")
        ...
        .show(ctx, |ui| {
            ...
            // --- Owned tags (explicit SD/HD) + optional Airing status ---
            {
                ...
                let broadcast_hd = crate::app::utils::infer_broadcast_hd(tags_joined.as_deref(), row.channel.as_deref());
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    ui.add(
                        eg::Label::new(
                            eg::RichText::new(if broadcast_hd { "Airing HD" } else { "Airing SD" })
                                .color(if broadcast_hd { eg::Color32::from_rgb(120, 180, 255) } else { eg::Color32::GRAY })
                        ).wrap()
                    );

...
impl crate::app::PexApp {
    pub(crate) fn ui_render_detail_panel(&mut self, ctx: &eg::Context) {
        ...
        let snapped_default: f32 = ((self.detail_panel_width / step).round() * step)
            .clamp(260.0, snapped_max);
            ...
            .show(ctx, |ui| {
                ...
                if row.channel.is_some() || row.airing.is_some() {
                    let ch = row
                        ...
                        .unwrap_or_else(|| "—".into());
                    let tm = row.airing.map(crate::app::utils::hhmm_utc).unwrap_or_else(|| "—".into());
                    ui.label(eg::RichText::new(format!("{ch}  •  {tm} UTC")).weak());
                }

                // Owned
                if row.owned {
                    ui.label(eg::RichText::new("Owned").color(eg::Color32::from_rgb(130, 200, 130)));
                }

                // HD status line
                {
                    let tags_joined = if !row.genres.is_empty() { Some(row.genres.join("|")) } else { None };
                    let is_broadcast_hd = crate::app::utils::infer_broadcast_hd(tags_joined.as_deref(), row.channel.as_deref());
                    ...
                    if better_hd_available {
                        ui.label(eg::RichText::new("Better HD available (upcoming)").color(eg::Color32::from_rgb(200, 200, 80)));
                    } else if is_broadcast_hd {
                        ui.label(eg::RichText::new("Broadcast: HD").weak());
                    }
                }

...
if (-not $IsWindows) {
    throw "release_windows.ps1 must be run from Windows."
}
...
Write-Host "=== Pex Release Helper (Windows) ===" -ForegroundColor Cyan
...
if (Ask-YesNo "Run build & packaging pipeline?" "Y") {
    ...
    $portableScript = Join-Path "make_portable" "make_portable.ps1"
    ...
    & $portableScript -Zip:$true -ZipName $zipName -OutputDir $distSubDir
...
if ($repoSlug) {
    try {
        ...
        if ($UpdateDocs -or $autoDocs) {
            ...
            Write-Host "Docs update skipped (run again once both platform bundles are available or pass -UpdateDocs)." -ForegroundColor Yellow
        }
    }
    catch {
...
Write-Host "`nWindows release helper finished." -ForegroundColor Cyan
